...
`dwyt_codebase` is **Code Intelligence**: structure, not memory. For decisions, task history, and handoff context, follow the [Obsidian Law](06-obsidian-law.md). For how much of a graph answer belongs in the context window, that decision belongs to `dwyt_optimizer` — see [Architecture v5](03-architecture-v5.md).
