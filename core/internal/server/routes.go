...
func registerRoutes(r *gin.Engine, ds *DashboardServer) {
	...
	api.Use(localOriginGuard(ds.Port))
	{
		...
		// DWYT MCP Governor (v5). The stdio MCP shim is a thin client of these
		// endpoints so every AI client shares one governor state.
		api.POST("/governor/plan", ds.apiGovernorPlan)
		api.GET("/governor/status", ds.apiGovernorStatus)
		api.POST("/governor/register", ds.apiGovernorRegister)
		api.GET("/governor/output-profile", ds.apiGovernorOutputProfile)
		api.GET("/governor/cache-guidance", ds.apiGovernorCacheGuidance)
		api.POST("/governor/compact", ds.apiGovernorCompact)
		api.GET("/governor/raw", ds.apiGovernorRaw)
