...
export default function SetupWizard() {
  ...
  async function handleSave() {
    ...
    try {
      const r = await fetch('http://127.0.0.1:2737/api/setup/install', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tools, ias, providers: [], project_path: projectPath }),
      })
      const data = await r.json()
      console.log('Install results:', data)
    } catch (e) {
